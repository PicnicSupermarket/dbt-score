...
def test_manifest_formatter_project_seeds_and_macros(
    capsys,
    default_config,
    manifest_loader,
    seed1,
    ...
    """Ensure seeds and macros also get their score written to the manifest."""
    formatter = ManifestFormatter(
        manifest_loader=manifest_loader, config=default_config
    )
    result1: EvaluableResultsType = {
    ...
    formatter.evaluable_evaluated(seed1, result1, Score(5.0, "🚧"))
    formatter.evaluable_evaluated(seed2, result2, Score(10.0, "🥇"))
    formatter.evaluable_evaluated(macro1, result1, Score(5.0, "🚧"))
    formatter.evaluable_evaluated(macro2, result2, Score(10.0, "🥇"))
    formatter.project_evaluated(Score(7.5, "🥉"))

    stdout = capsys.readouterr().out
    new_manifest = json.loads(stdout)

    assert new_manifest["nodes"]["seed.package.seed1"]["meta"]["score"] == 5.0
    assert new_manifest["nodes"]["seed.package.seed1"]["meta"]["badge"] == "🚧"
    assert new_manifest["nodes"]["seed.package.seed2"]["meta"]["score"] == 10.0
    assert new_manifest["nodes"]["seed.package.seed2"]["meta"]["badge"] == "🥇"

    assert new_manifest["macros"]["macro.package.macro1"]["meta"]["score"] == 5.0
    assert new_manifest["macros"]["macro.package.macro1"]["meta"]["badge"] == "🚧"
    assert new_manifest["macros"]["macro.package.macro2"]["meta"]["score"] == 10.0
    assert new_manifest["macros"]["macro.package.macro2"]["meta"]["badge"] == "🥇"
